...
class LLVMCodeBr(LLVMCode):
    ...
    def __init__(self, cond:Operand=None, label1:Operand=None, label2:Operand=None):
        super().__init__()
        self.cond = cond
        self.label1 = label1
        self.label2 = label2
    ...
    def __str__(self):
        if self.cond != None:
            return f"br i1 {self.cond}, label %{self.label1}, label %{self.label2}"
        else:
...
class LLVMCodeCallStatement(LLVMCode):
    ''' 関数呼び出し
        call void @{fundef.name}()
    '''

    def __init__(self,fundef:Fundef):
        super().__init__()
        self.fundef = fundef

    def __str__(self):
        return f"call void @{self.fundef.name}()"
